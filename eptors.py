...
def res_obtain(obs, eph):
    ...
    n_toas = len(open(obs,'rU').readlines())
    os.environ['var']=str(n_toas)
  

    os.system("tempo2 -output general2 -f %s %s_tmp -npsr 1 -nobs $n_toas -s \
...
def show_residual(obs, eph):
    n_toas = len(open(obs,'rU').readlines())
    os.environ['var']=str(n_toas)
    subprocess.call('tempo2 -gr plk -f %s %s -npsr 1 -nobs $n_toas -showchisq'%(eph, obs), shell=True)
...
def trim(obs,eph):
    ...
    trim_del_toa = post[out_mask]
    trim_del_err = err[out_mask]
    trim_del_freq= freq[out_mask]
    
    trim_keep_freq = freq[in_mask]
    trim_keep_toa = post[in_mask]
    trim_keep_err = err[in_mask]
...
def trim_plot(obs, eph):    
    freq, post, err = res_obtain(obs, eph)
    trim_del_toa, trim_del_err,trim_del_freq,_,_,_ = trim(obs,eph)


    #Plot the orignal residuals
    plt.errorbar(freq,post,yerr=err,fmt='b.')
    ...
    plt.errorbar(trim_del_freq, trim_del_toa, yerr=trim_del_err,fmt='k.')
...
def gauss_fit(obs, eph):       
    ...
    gaus_keep_toa = post[in_mask]
    gaus_keep_err = err[in_mask]
    
    gaus_del_toa = post[out_mask]
    gaus_del_err = err[out_mask]
    gaus_del_freq = freq[out_mask]
    ...
    os.system("rm %s_tmp"%(obs))
    plt.close()
    return gaus_del_freq, gaus_del_toa, gaus_del_err, gaus_keep_toa, gaus_keep_err, 


def gauss_plot(obs, eph):
    freq, post, err = res_obtain("trim.tim", eph)
    gaus_del_freq, gaus_del_toa, gaus_del_err, _, _  =  gauss_fit("trim.tim", eph)    
    
    plt.errorbar(freq,post,yerr=err,fmt='k.')    
    plt.errorbar(gaus_del_freq, gaus_del_toa, yerr=gaus_del_err,fmt='c.')
...
def showeffect(obs, eph, algo): 
    ...
    if algo == "t":   
        ...
        _,gaus_del_toa, gaus_del_err, gaus_keep_toa, gaus_keep_err = gauss_fit("trim.tim", eph)
